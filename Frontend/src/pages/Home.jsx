...
export default function Home() {
  ...
  if (isLoading || !user) { 
    return (
      <main className="flex-1 p-4 md:p-8 md:ml-64 lg:ml-72 xl:ml-80 2xl:ml-[22rem] pb-20 md:pb-8 flex items-center justify-center min-h-screen">
  ...
  return (
    <>
      ...
      {/* 
        RESPONSIVE LAYOUT:
        - Mobile: No left margin, full width, bottom padding for nav (pb-20)
        - Tablet/Desktop: Left margin for sidebar, less bottom padding (pb-8)
      */}
      <main className="flex-1 p-3 sm:p-4 md:p-6 lg:p-8 md:ml-64 lg:ml-72 xl:ml-80 2xl:ml-[22rem] pb-20 md:pb-8 transition-all duration-300">
        <div className="max-w-2xl mx-auto space-y-4 md:space-y-6">
          ...
          {/* --- MOMENTS --- */}
          <div className="rounded-2xl p-3 sm:p-4 md:p-6 border border-secondary/20 overflow-hidden bg-gradient-to-br from-purple-50 to-purple-100">
            <div>
              <h2 className="text-white font-bold px-3 sm:px-4 py-2 rounded-full inline-block mb-3 md:mb-4 text-sm sm:text-base md:text-lg bg-gradient-to-r from-[#1D0C69] to-[#5A0395]">
                MOMENTS
              </h2>
              <div className="flex gap-3 sm:gap-4 overflow-x-auto pb-2 scrollbar-hide">
                ...
                <div className="flex-shrink-0">
                  <Link to="/create?tab=moment" className="flex flex-col items-center gap-2">
                    <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-full p-[2px] bg-gradient-to-br from-[#1D0C69] to-[#5A0395] cursor-pointer transition-shadow hover:shadow-xl flex items-center justify-center">
                       <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-full bg-white/75 backdrop-blur-sm border border-white/20 flex items-center justify-center">
                         <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-[#5A0395] text-white flex items-center justify-center shadow-md">
                           <Plus className="w-4 h-4 sm:w-5 sm:h-5" />
                         </div>
                       </div>
                    </div>
                    <span className="text-xs text-[#5A0395] font-medium whitespace-nowrap">Add Moment</span>
                  </Link>
                </div>
                ...
                {userStories.length > 0 && (
                  <div className="flex flex-col items-center gap-2 flex-shrink-0">
                    ...
                    <span className="text-xs text-[#5A0395] font-medium whitespace-nowrap">Your Story</span>
                  </div>
                )}
                ...
                {moments.map((group) => (
                  <div key={group.userId} className="flex flex-col items-center gap-2 flex-shrink-0">
                    ...
                    <span className="text-xs text-[#5A0395] font-medium whitespace-nowrap">{group.username}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="space-y-4 md:space-y-6">
            <h2 className="text-white font-bold gradient-sidebar px-3 sm:px-4 py-1 rounded-full inline-block text-sm sm:text-base">
              FEED
            </h2>
            ...
            {posts.length === 0 && (
              <Card className="overflow-hidden rounded-2xl border border-secondary/20 p-0 bg-gradient-to-br from-[#1D0C69] to-[#5A0395]">
                <div className="p-8 sm:p-10 text-center text-white">
                  <p className="text-base sm:text-lg">Your feed is empty.</p>
                  <p className="text-xs sm:text-sm mt-1">Follow some users to see their posts here!</p>
                </div>
              </Card>
            )}

            {posts.map((post) => (
              <Card key={post.post_id} className="overflow-hidden shadow-lg border w-full">
                {/* Header */}
                <div className="flex items-center justify-between p-2 sm:p-3 bg-gradient-to-r from-[#1D0C69] to-[#5A0395] border-b border-purple-600">
                  <div 
                  ...
                  >
                    ...
                    <div className="min-w-0">
                      <p className="font-semibold truncate text-white text-sm sm:text-base">{post.username}</p>
                    </div>
                  </div>
                </div>

                {/* Media */}
                <div className="bg-black max-h-[400px] sm:max-h-[500px] flex items-center justify-center border-b">
                  {post.media && post.media.length > 0 ? (
                    post.media[0].media_type.startsWith('video') ? (
                      <video src={post.media[0].media_url} controls className="w-full max-h-[400px] sm:max-h-[500px] object-contain" />
                    ) : (
                      <img src={post.media[0].media_url} alt="Post media" className="w-full max-h-[400px] sm:max-h-[500px] object-contain" />
                    )
                  ) : (
                    <span className="text-4xl sm:text-6xl font-bold text-muted-foreground/20">MEDIA</span>
                  )}
                </div>

                {/* Caption & Actions */}
                <div className="p-3 sm:p-4 space-y-2 sm:space-y-3 bg-gradient-to-br from-purple-50 to-purple-100">
                  <div className="flex items-start justify-between gap-2">
                    <p className="text-xs sm:text-sm break-words flex-1 min-w-0 text-gray-800">
                    ...
                    <div className="flex items-center gap-2 sm:gap-3 text-gray-600 flex-shrink-0">
                      ...
                      >
                        <ThumbsUp className={`w-4 h-4 sm:w-5 sm:h-5 ${likedPosts.includes(post.post_id) ? "fill-current" : ""}`} />
                        <span className="text-xs sm:text-sm ml-1">{post.like_count}</span>
                      </Button>
                      <Button 
                        variant="ghost" 
                        size="icon" 
                      ...
                      >
                        ...
                        {post.comment_count > 0 && (
                          <span className="absolute -top-1 -right-1 bg-[#5A0395] text-white text-[10px] sm:text-xs rounded-full w-3.5 h-3.5 sm:w-4 sm:h-4 flex items-center justify-center">
                            {post.comment_count}
                          </span>
                        )}
                      </Button>
                      ...
                      >
                        <Bookmark className={`w-4 h-4 sm:w-5 sm:h-5 ${savedPosts.includes(post.post_id) ? "fill-current" : ""}`} />
                      </Button>
                    </div>
                  </div>
                  ...
                  {post.hashtags && post.hashtags.length > 0 && (
                    <div className="border-t pt-2 sm:pt-3 border-purple-200">
                      <p className="font-semibold text-xs sm:text-sm mb-1 text-[#1D0C69]">#Hashtags:</p>
                      <div className="flex flex-wrap gap-1.5 sm:gap-2">
                        {post.hashtags.map((tag, idx) => {
                          ...
                          return (
                            <span 
                              key={idx} 
                              className="text-xs sm:text-sm text-[#5A0395] hover:underline cursor-pointer font-medium" 
                  ...
                  <p className="text-[10px] sm:text-xs text-gray-600">{formatTimeAgo(post.created_at)}</p>
                </div>
